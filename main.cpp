...
void sha1(std::string data, uint32_t* output) {
    ...
    data += (char)0b10000000;

    while ((data.size() * 8) % 512 != 448) {
        data += (char)0;
    }
    data += (char)((ml & 0xFF00000000000000) >> 56);
    data += (char)((ml & 0x00FF000000000000) >> 48);
    data += (char)((ml & 0x0000FF0000000000) >> 40);
    data += (char)((ml & 0x000000FF00000000) >> 32);
    data += (char)((ml & 0xFF000000) >> 24);
    data += (char)((ml & 0x00FF0000) >> 16);
    data += (char)((ml & 0x0000FF00) >> 8);
    data += (char)(ml & 0x000000FF);
...
//        if (bits % 32 == 0) {
//            chunks.back().push_back(0);
//        }

        chunks.back()[bits / 32] |= ((ch << (24 - (bits % 32))) & (0xFF << (24 - (bits % 32))));
...
//    std::cout << '\n';
    ...
    for (auto& chunk : chunks) {
        ...
        for (int i = 16; i < 80; i++) {
            chunk[i] = rotateleft((chunk[i - 3] ^ chunk[i - 8] ^ chunk[i - 14] ^ chunk[i - 16]), 1);
        }
